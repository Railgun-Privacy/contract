  ...
  return [
    {
      ...
      data: rail.interface.encodeFunctionData('balanceOf(address)', [
...
async function becomeWhale() {
  ...
  await ethers.provider.send('hardhat_setStorageAt', [DEPLOYCONFIG.rail, index, newBalance]);

  await ethers.provider.send('evm_mine');
}

async function fastForward(days) {
  await ethers.provider.send('evm_increaseTime', [Math.round(86400 * days)]);
...
async function stakeAll() {
  ...
  (await staking.stake(await rail.balanceOf((await ethers.getSigners())[0].address))).wait();
}
...
async function adminDeployToRopsten() {
  ...
  for (let i = 0; i < calls.length; i += 1) {
    ...
    await // eslint-disable-next-line no-await-in-loop
    (await delegator.callContract(calls[i].callContract, calls[i].data, calls[i].value)).wait();
  }
}
