...
/**
 * Exports Storage Layouts to file
 *
 * @param hre - hardhat runtime environment
 * @param contracts - contracts to export layouts for
 * @returns complete
 */
async function exportStorageLayouts(
  hre: HardhatRuntimeEnvironment,
  contracts: string[],
): Promise<void> {
  // Get output directory and ensure it exists
  ...
  await Promise.all(
    contracts.map(async (contractName) => {
      ...
      const storageLayout = info.output.contracts[file][contract].storageLayout as ContractStorage;
...
/**
 * Deletes exported storage layouts folder
 *
 * @param hre - hardhat runtime environment
 * @returns complete
 */
function cleanExportedStorageLayouts(hre: HardhatRuntimeEnvironment) {
  const outputDirectory = path.resolve(hre.config.paths.root, './storage-layouts');
  if (!fs.existsSync(outputDirectory)) return null;
  fs.rmSync(outputDirectory, { recursive: true, force: true });
  return null;
