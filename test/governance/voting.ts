...
let voting: Contract;
let target: Contract;
let target2: Contract;

// Contract parameters
let sponsorWindow: number;
let votingStartOffset: number;
let votingYesEndOffset: number;
let votingNoEndOffset: number;
let executionStartOffset: number;
let executionEndOffset: number;
let quorum: number;
let proposalSponsorThreshold: number;
...
describe('Governance/Voting', () => {
  beforeEach(async () => {
    ...
    await staking.stake(await testERC20.balanceOf((await ethers.getSigners())[0].address));
  ...
  it('Should go through vote lifecycle correctly', async () => {
    ...
    await expect(
      voting.unsponsorProposal(0n, proposalSponsorThreshold)
    ...
    await ethers.provider.send('evm_increaseTime', [
      Number(votingYesEndOffset.toString()) - Number(votingStartOffset.toString()),
    ]);
    await ethers.provider.send('evm_mine', []);
    ...
    await ethers.provider.send('evm_increaseTime', [
      Number(votingNoEndOffset.toString()) - Number(votingYesEndOffset.toString()),
    ]);
    await ethers.provider.send('evm_mine', []);
    ...
    await ethers.provider.send('evm_increaseTime', [
      Number(executionStartOffset.toString()) - Number(votingNoEndOffset.toString()),
    ]);
    await ethers.provider.send('evm_mine', []);
    ...
    await ethers.provider.send('evm_increaseTime', [
      Number(executionEndOffset.toString()) - Number(votingNoEndOffset.toString()),
    ]);
    await ethers.provider.send('evm_mine', []);
  ...
  it('Should not be able to sponsor after the sponsor window', async () => {
    ...
    await ethers.provider.send('evm_increaseTime', [Number(sponsorWindow.toString())]);
    await ethers.provider.send('evm_mine', []);

    // Sponsor should fail
    await expect(
      voting.sponsorProposal(0n, proposalSponsorThreshold, 0n)
    ).to.eventually.be.rejectedWith('Voting: Sponsoring window passed');
  });
  ...
  it('Should not not execute failed proposal', async () => {
    ...
    // Increase time to execution window start
    await ethers.provider.send('evm_increaseTime', [
      Number(executionStartOffset.toString()) - Number(votingStartOffset.toString()),
    ]);
    await ethers.provider.send('evm_mine', []);
