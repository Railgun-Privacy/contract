...
describe('Logic/TokenBlacklist', () => {
  ...
  it('Should add and remove from blacklist', async () => {
    expect(
      await tokenBlacklist.tokenBlacklist('0xeb4c2781e4eba804ce9a9803c67d0893436bb27d')
    ).to.equal(false);
    expect(
      await tokenBlacklist.tokenBlacklist('0x3832d2F059E55934220881F831bE501D180671A7')
    ).to.equal(false);
    expect(
      await tokenBlacklist.tokenBlacklist('0x459086f2376525bdceba5bdda135e4e9d3fef5bf')
    ).to.equal(false);
    expect(
      await tokenBlacklist.tokenBlacklist('0x1c5db575e2ff833e46a2e9864c22f4b22e0b37c2')
    ).to.equal(false);
  ...
  it('Should emit blacklist events', async () => {
    ...
    (
      ...
      expect((event.args as any).token.toLowerCase()).to.equal(expected[index].toLowerCase());
    });
    ...
    expected = ['0xeb4c2781e4eba804ce9a9803c67d0893436bb27d'];

    (
      await (
        await tokenBlacklist.removeFromBlacklist(['0xeb4c2781e4eba804ce9a9803c67d0893436bb27d'])
      ).wait()
    ).events.forEach((event: Event, index: number) => {
